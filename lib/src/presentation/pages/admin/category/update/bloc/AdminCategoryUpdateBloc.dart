...
class AdminCategoryUpdateBloc extends Bloc<AdminCategoryUpdateEvent, AdminCategoryUpdateState> {
  ...
  AdminCategoryUpdateBloc(this.categoryUseCase) : super(const AdminCategoryUpdateState()) {
  ...
  void _onInit(AdminCategoryUpdateInitEvent event, Emitter<AdminCategoryUpdateState> emit) async{
    emit(state.copyWith(
  ...
  Future<void> _onFormSubmit(FormSubmit event, Emitter<AdminCategoryUpdateState> emit) async {
    ...
    Resource response = await categoryUseCase.updateCategoryUseCase.run(
      state.id, 
      state.toCategory(), 
      state.file  
    );
  ...
  void _onPickImage(PIckImage event, Emitter<AdminCategoryUpdateState> emit) async {
  ...
  void _onTakePhoto(TakePhoto event, Emitter<AdminCategoryUpdateState> emit) async {
    final ImagePicker picker = ImagePicker();
